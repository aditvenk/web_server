...
void requestHandleInitial (int fd, work_queue_t* job) {
   ...
   strcpy(job->cgiargs, cgiargs);

   if (stat(job->filename, &sbuf) < 0) {
      requestError(fd, job->filename, "404", "Not found", "CS537 Server could not find this file");
      return;
   }
   job->job_length = sbuf.st_size;
   //printf("job %d, job_length = %d ...", job->connfd, job->job_length);
}
...
void requestHandleFinal ( work_queue_t* job) {
   ...
   if (is_static) {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IRUSR & sbuf.st_mode)) {
         requestError(fd, job->filename, "403", "Forbidden", "CS537 Server could not read this file");
         return;
      }
      requestServeStatic(fd, job->filename, sbuf.st_size);
   } else {
      if (!(S_ISREG(sbuf.st_mode)) || !(S_IXUSR & sbuf.st_mode)) {
         requestError(fd, job->filename, "403", "Forbidden", "CS537 Server could not run this CGI program");
         return;
      }
      requestServeDynamic(fd, job->filename, job->cgiargs);
   }
} 
// handle a request
